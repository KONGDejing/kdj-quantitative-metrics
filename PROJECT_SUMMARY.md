...
- **邮件通知**（`src/notifier.py`）：SMTP SSL 发送，当前配置 163 邮箱发件，同时发送到 QQ 邮箱和 163 邮箱。
...
- **安全整改**：`config.yaml`（含 SMTP 授权码）从 git 移除并加入 `.gitignore`，改用 `config.example.yaml` 模板提交；`best_params.json`、`cache/` 等运行产物一并排除。
