...
- 通知：邮件和 Pushplus 微信双通道；盘中极值提醒、基于新鲜实时行情的目标价提醒、14:50 收盘前总结、15:15 后次日操作指引。
- 可信交易账本：Web 录入成交时区分核心仓/T仓，逐笔重算持仓、平均买入成本、保本成本、已实现收益和 T+1 可卖手数。
- 确定性交易计划：正式日线信号依次通过账本、基本面、回撤、资金、阶段仓位和止跌确认闸门，输出动作、最多手数、价位和取消条件。
- 简单冲高反T：较前收冲高约1.8%且10分钟K从80以上拐头时，单轮最多卖2手可卖老仓；盈利回补也按实际卖价下方约1.8%。总额度20%、至少保留8手、单日最多一轮，MA趋势过滤代码保留但当前关闭。
...
- 安全写入：可信内网单人部署可直接录入；公网或多人网络可开启写入令牌。成交支持先校验整本账本再替换/删除，错误值不进入审计历史。
...
`price_alerts` 使用独立实时快照，不拿旧K线充数：只有行情日期为当天且时间戳未超过
`max_quote_age_seconds` 才会触发。价格进入“目标价 + tolerance_ratio”区域时推送一次，
上涨到“目标价 + reset_ratio”之外后才重新布防；`only_when_flat: true` 可避免已经持仓后
继续发送“首次试仓”提醒。系统仍然不会自动下单。
...
写接口支持两种模式。`web.require_write_token: true`适合公网或多人网络，所有持久化写操作必须携带`X-API-Key`，页面会校验浏览器保存的令牌；令牌可通过`./scripts/show-write-token.sh`获取。`web.require_write_token: false`适合可信内网单人使用，页面可直接添加/删除股票、录入和纠正成交。页面切换股票始终无需令牌。关闭令牌保护意味着同一网络内能访问页面的人也可以修改数据，请只在可信网络使用。

成交录入带同日语义去重：同一股票、方向、手数、价格和仓位类别已存在时，默认返回冲突且不写入；页面只有在用户明确确认“这是另一笔独立成交”后才允许再次保存。提交期间按钮会禁用，避免双击重复。
